	...
	let rec u_help list t = function
  	| Var a ->
  		...
  		| x when (not (isIn a (tv x))) -> Success (List.rev ((a,x)::list))
			| _ -> Error (sprintf "Found type '%s' where type '%s' was expected." (string_of_type (Var a)) (string_of_type t)))
  	| Imp (t1,t2) ->
			(match t with
			| Imp (s1, s2) ->
			...
			| _ -> Error (sprintf "Found type '%s' where type '%s' was expected." (string_of_type (Imp (t1,t2))) (string_of_type t)))
  	| Tup (t1,t2) ->
			(match t with
			| Tup (s1, s2) ->
				(match u_help [] s2 t2 with
			...
			| _ -> Error (sprintf "Found type '%s' where type '%s' was expected." (string_of_type (Tup (t1,t2))) (string_of_type t)))
  	| Lis t1 ->
			(match t with
			| Lis s1 -> u_help [] s1 t1
			| Var a when (not (isIn a (tv (Lis t1)))) -> Success (List.rev ((a,(Lis t1))::list))
			| _ -> Error (sprintf "Found type '%s' where type '%s' was expected." (string_of_type (Lis t1)) (string_of_type t)))
  	| t1 ->
			(match t with
			| Var a when (not (isIn a (tv t1))) -> Success (List.rev ((a,t1)::list))
			| t2 when (t1 = t2) -> Success (List.rev list)
			| _ -> Error (sprintf "Found type '%s' where type '%s' was expected." (string_of_type t1) (string_of_type t))) in
	u_help [] t1 t2;;
