...
let fid_to_mid i fid = "|"^(string_of_int i)^fid

let rec get_matchvars fid i = function
	| (Stmt_match (exp,case_list))::stmt_list -> (None,fid_to_mid i fid,exp)::(get_matchvars fid (i+1) stmt_list)
	| _::stmt_list -> get_matchvars fid i stmt_list
	| [] -> []
...
let merge_extracts extract1 extract2 : stelling_extract =
...
let rmc_stelling mid extract : stelling_extract = function
...
let rmc_hyperlocalvar id (vars : (id*varbeschrijving) list) : varbeschrijving =
	try
		snd (List.find (fun (x : (id*varbeschrijving)) -> fst x = id))
	with
	| Not_found -> (None,Nofield id);;

let rec rmc_whenpredicate_fieldexp vars : varbeschrijving = function
...
let rmc_whenpredicate vars : exp = function
	| Exp_field fieldexp ->
...
let rmc_case mid : (exp * stmt list) = function
...
let rmc_stmt_list fid i : (stmt list)= function
	| (Stmt_match (exp, case_list))::stmt_list -> append (rmc_match_stmt fid i (exp,case_list)) rmc_stmt_list fid (i+1) stmt_list
	| head::tail -> head::(rmc_stmt_list fid i tail)
	| [] -> []

let rmc_fundecl : fundecl = function
	| (fid,fargs,funtype,vardecl_list,stmt_list) -> 
		let vardecl_list = List.append vardecl_list (get_matchvars fid 0 stmt_list) in
		let stmt_list = rmc_stmt_list fid 0 stmt_list in
		(fid,fargs,funtype,vardecl_list,stmt_list)
		
let rmc_spl : spl = function
